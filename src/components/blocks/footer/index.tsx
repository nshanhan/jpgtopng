    ...
    <section id={footer.name} className="py-16">
      <div className="max-w-7xl mx-auto px-8">
        <footer>
          ...
          <div className="mt-8 flex flex-col justify-between gap-4 border-t pt-8 text-center text-sm font-medium text-muted-foreground  lg:items-center lg:text-left">
            {footer.agreement && (
              <ul className="flex justify-center  lg:justify-start gap-5">
                {footer.agreement.items?.map((item, i) => (
                  <li key={i} className="hover:text-primary px-2 py-4 ">
                    <a href={item.url} target={item.target}>
                      {item.title}
                    </a>
                  </li>
            ...
            {footer.copyright && (
              <p className="text-[#333333]">{footer.copyright}</p>
            )}
          </div>
        </footer>
      </div>
